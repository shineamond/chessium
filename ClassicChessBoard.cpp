...
void ClassicChessBoard::DrawDefaultColorSquare(const int row, const int col) const
{
...
//void ClassicChessBoard::DrawClickedSquare(const int row, const int col) const
//{
//    SDL_Rect drawing_position{col * _SQUARE_SIZE, 60 + row * _SQUARE_SIZE, _SQUARE_SIZE, _SQUARE_SIZE};
...
//
//
//void ClassicChessBoard::DrawPiece(const int row, const int col) const
//{
...
void ClassicChessBoard::SetupDefaultBoard()
{
...
//void ClassicChessBoard::DrawChessBoardAndPieces() const
//{
//    for (int row = 0; row < _BOARD_SIZE; row++)
//    {
//        for (int col = 0; col < _BOARD_SIZE; col++)
...
_MESSAGES_FROM_CHESSBOARD ClassicChessBoard::HandleGame(const _CHESS_PIECE_COLORS side_to_move)
{
        ...
        if ((clicked_square_row * _SQUARE_SIZE + 60 != mouse_position_y) && (clicked_square_col * _SQUARE_SIZE != mouse_position_x) && (clicked_square_row < _BOARD_SIZE) && (clicked_square_col < _BOARD_SIZE))
        {
            ...
            else // A square has already been clicked
            {
                ...
                if (clicked_square_row == clicked_squares_list_[0].first && clicked_square_col == clicked_squares_list_[0].second) // Click the clicked square
                {
                    ...
                    if (pieces_positions_[clicked_square_row][clicked_square_col] -> GetPieceType() == _KING)
                    {
                        DrawSquareAndPieceIfKingInCheck(pieces_positions_[clicked_square_row][clicked_square_col] -> GetPieceColor());
                    }
...
//                            }
                            ...
                            if (side_to_move == _WHITE)
                            {
                                ...
                                DrawSquareAndPieceIfKingInCheck(_BLACK);

                                DrawDefaultColorSquare(GetWhiteKingPosition().first, GetWhiteKingPosition().second);
                                DrawPiece(GetWhiteKingPosition().first, GetWhiteKingPosition().second);
                            }
                            else if (side_to_move == _BLACK)
                            {
                                ...
                                DrawSquareAndPieceIfKingInCheck(_WHITE);

                                DrawDefaultColorSquare(GetBlackKingPosition().first, GetBlackKingPosition().second);
                                DrawPiece(GetBlackKingPosition().first, GetBlackKingPosition().second);
                            }
...
//                    if (!(is_move || is_capture || is_castle))
                    {
                        if (pieces_positions_[clicked_square_row][clicked_square_col] != nullptr)
                        {
                            if (pieces_positions_[clicked_square_row][clicked_square_col] -> GetPieceColor() == side_to_move)
                            {
                                ...
                                if (pieces_positions_[clicked_squares_list_[0].first][clicked_squares_list_[0].second] -> GetPieceType() == _KING)
                                {
...
bool ClassicChessBoard::PromotePawn(const int col, const _CHESS_PIECE_COLORS color)
{
...
pair <int, int> ClassicChessBoard::GetWhiteKingPosition() const
{
...
pair <int, int> ClassicChessBoard::GetBlackKingPosition() const
{
    for (int row = 0; row < _BOARD_SIZE; row++)
    {
        for (int col = 0; col <_BOARD_SIZE; col++)
...
bool ClassicChessBoard::IsKingInCheck(const _CHESS_PIECE_COLORS king_color)
{
...
bool ClassicChessBoard::IsLegalMove(const int old_row, const int old_col, const int new_row, const int new_col, const _MOVE_TYPES move_type, const _CHESS_PIECE_COLORS side_to_move)
{
    bool is_legal = true;
...
void ClassicChessBoard::AddLegalCastling(const _CHESS_PIECE_COLORS side_to_move)
{
    switch (side_to_move)
    {
        case _BLACK:
...
bool ClassicChessBoard::AddEnPassantMoves(const _CHESS_PIECE_COLORS side_to_move)
{
...
bool ClassicChessBoard::IsDraw50Moves()
{
...
bool ClassicChessBoard::IsDrawThreefoldRepetition()
{
    int log_size = moves_log_.size();
...
bool ClassicChessBoard::IsDrawInsufficientMaterials()
{
...
pair<int, int> ClassicChessBoard::FindBishopPosition(const _CHESS_PIECE_COLORS bishop_color)
{
    for (int row = 0; row < _BOARD_SIZE; row++)
    {
        for (int col = 0; col < _BOARD_SIZE; col++)
...
bool ClassicChessBoard::IsOnlyKingLeft(const _CHESS_PIECE_COLORS side) const
{
    if (side == _WHITE)
    {
...
void ClassicChessBoard::ResetBoard()
{
    for (int row = 0; row < _BOARD_SIZE; row++)
    {
        for (int col = 0; col < _BOARD_SIZE; col++)
...
void ClassicChessBoard::DrawSquareAndPieceIfKingInCheck(const _CHESS_PIECE_COLORS king_color)
{
    pair <int, int> king_position;
    if (king_color == _WHITE)
    {
        king_position = GetWhiteKingPosition();
        if (!IsKingInCheck(_WHITE))
        {
            return;
        }
    }
    else if (king_color == _BLACK)
    {
        king_position = GetBlackKingPosition();
        if (!IsKingInCheck(_BLACK))
        {
            return;
        }
    }

    SDL_Rect drawing_position{king_position.second * _SQUARE_SIZE, 60 + king_position.first * _SQUARE_SIZE, _SQUARE_SIZE, _SQUARE_SIZE};
    SDL_SetRenderDrawColor(RENDERER, 255, 0, 7, 200); // RED
    SDL_RenderFillRect(RENDERER, &drawing_position);
    DrawPiece(king_position.first, king_position.second);
}
